...
export default function HiddenContent() {
  useEffect(() => {
    console.clear();
    const hiddenContent = document.querySelector(`.${styles.hiddenContent}`);
    const content = document.querySelectorAll(`.${styles.text}`);
    const link = document.querySelector(`.${styles.link}`);
    const linkIcon = document.querySelector(`.${styles.btnIcon}`);
    let linkAnimated = false;

    let xTo = gsap.quickTo(hiddenContent, "--x", { duration: 0.4, ease: "power4.out" });
    let yTo = gsap.quickTo(hiddenContent, "--y", { duration: 0.4, ease: "power4.out" });

    let tl = gsap.timeline({ paused: true });
    tl.to(hiddenContent, { "--size": 250, duration: 0.75, ease: "back.out(1.7)" });
    ...
    btnTl.to(hiddenContent, { "--size": 20, duration: 0.75, ease: "back.out(1.7)" });
      ...
      <div className={styles.content}>
        <div className={styles.container}>
          ...
          <div className={styles.textCenter}>
            <a className={styles.link} href="https://greensock.com/" target="_blank" rel="noopener noreferrer">
