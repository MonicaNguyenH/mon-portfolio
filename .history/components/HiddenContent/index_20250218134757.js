...
export default function HiddenContent() {
  useEffect(() => {
    ...
    let xTo = gsap.quickTo(`.${styles.hiddenContent}`, "--x", { duration: 0.4, ease: "power4.out" });
    let yTo = gsap.quickTo(`.${styles.hiddenContent}`, "--y", { duration: 0.4, ease: "power4.out" });

    let tl = gsap.timeline({ paused: true });
    tl.to(`.${styles.hiddenContent}`, { "--size": 250, duration: 0.75, ease: "back.out(1.7)" });

    const hoveringContent = document.querySelectorAll(`.${styles.text}`);
    ...
    btnTl.to(`.${styles.hiddenContent}`, { "--size": 20, duration: 0.75, ease: "back.out(1.7)" });
