...
export default function BeepOverview() {
  ...
  return (
    <div className="relative w-full h-screen flex items-center justify-center bg-black text-white overflow-hidden">
      {/* Background Text (Masked and Revealed on Hover) */}
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
        <defs>
          <mask id="hover-mask">
            ...
            {hoveredShape && (
              <g fill="black">
                {hoveredShape === 'rect' && <rect x="10" y="10" width="40" height="40" />}
                {hoveredShape === 'circle' && <circle cx="70" cy="30" r="20" />}
                {hoveredShape === 'triangle' && <polygon points="40,70 60,90 80,70" />}
              </g>
            )}
          </mask>
        </defs>
      ...
      <div className="absolute inset-0 flex items-center justify-center text-4xl font-bold text-gray-500">
        <p className="masked-text">UX/UI DESIGN - PRODUCT DESIGN - USER TESTING - RESEARCH</p>
      </div>
