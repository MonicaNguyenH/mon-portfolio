        ...
        import("gsap").then(({ default: gsapModule }) => {
            ...
            import("gsap/ScrollTrigger").then(({ ScrollTrigger: ST }) => {
                ...
                if (comparisonSectionRef.current) {
                    ...
                    // ✅ First slide-in animation (After Image over Before Image)
                    tl.fromTo(
                        section.querySelector(`.${styles.beforeImageImage}`),
                        { x: "100%" }, // Starts offscreen
                        { x: "0%" }
                    ).fromTo(
                        section.querySelector(`.${styles.afterImage} img`),
                        { x: "-100%" }, // Move opposite to balance
                        { x: "0%" },
                        0
                    );

                    // ✅ Second slide-in animation (Third Image over After Image)
                    tl.fromTo(
