            ...
            import("gsap/ScrollTrigger").then(({ ScrollTrigger: ST }) => {
                ...
                if (comparisonSectionRef.current) {
                    ...
                    tl.fromTo(
                        ...
                        { clipPath: "inset(0 0 0 100%)" }, // Starts hidden on the left
                        { clipPath: "inset(0 0 0 0%)", duration: 1.5, ease: "power2.out" } // ✅ Smoother transition, longer duration
                    );
                    
                    // ✅ Delay before the second image starts revealing
                    tl.fromTo(
                        section.querySelector(`.${styles.thirdImage}`),
                        { clipPath: "inset(0 0 0 100%)" }, // Start hidden on the left
                        { clipPath: "inset(0 0 0 0%)", duration: 1.5, ease: "power2.out" }, // ✅ Slower reveal with delay
                        "+=0.5" // ✅ Added delay (0.5s) before third image starts
                    );
