...
export default function Beep() {
    ...
    useEffect(() => {
        if (!isComparisonReady) return; // ✅ Ensures images are loaded first
        ...
        }).then(({ ScrollTrigger: ST }) => {
            ScrollTrigger = ST;
            gsap.registerPlugin(ScrollTrigger);
    
            if (!comparisonSectionRef.current) {
                console.warn("Comparison section is not found, skipping GSAP animation.");
                return; // ✅ Prevents running GSAP on null elements
            }
    
            let section = comparisonSectionRef.current;
            let afterImage = section.querySelector(`.${styles.afterImage}`);
            let thirdImage = section.querySelector(`.${styles.thirdImage}`);
    
            if (!afterImage || !thirdImage) {
                console.warn("GSAP Target elements not found, skipping animations.");
                return; // ✅ Prevents animation errors if elements are missing
            }
