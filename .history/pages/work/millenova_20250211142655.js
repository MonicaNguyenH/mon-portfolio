...
export default function Millenova() {
    ...
    const [isMounted, setIsMounted] = useState(false); // ✅ Ensures GSAP runs only after mount

    useEffect(() => {
        setIsMounted(true); // ✅ Ensures useEffect only runs after hydration
    }, []);

    useEffect(() => {
        if (!isMounted || typeof window === "undefined") return; // ✅ Prevents SSR issues
        ...
        import("gsap").then(({ default: gsap }) => {
            import("gsap/ScrollTrigger").then(({ ScrollTrigger }) => {
                ...
                window.addEventListener("load", () => ScrollTrigger.refresh()); // ✅ Ensures proper refresh
            });
        });
    ...
    }, [isMounted]); // ✅ Runs only after mount
        ...
        <>
            ...
            <div className={styles.main}>
                <ProjectHeader 
                    ...
                    introduction="The Millenova exhibition poster draws inspiration from Dior's SS 2000 campaign, reinterpreted with bold visuals and innovative techniques."
                    tools={projectTools}
                    date="Sept 2024"
                />
